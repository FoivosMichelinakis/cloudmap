...
tracerouteQuery = """select experiment_id, traceroute_id, nodeid, interface, startTime, endTime
from cloudmap.tracerouteParameters
where (sufficientMetadataValues IS NULL OR NOT sufficientMetadataValues);"""
cur.execute(tracerouteQuery)
conn.commit()
rows = cur.fetchall()


for element in rows:
    [experiment_id, traceroute_id, nodeid, interface, startTime, endTime] = element
    ...
    if sufficientMetadataValues:
        cur.execute("""UPDATE cloudmap.tracerouteParameters
        ...
        WHERE traceroute_id = %s""",
        [sufficientMetadataValues, traceroute_id])
    conn.commit()
cur.close()
conn.close()
